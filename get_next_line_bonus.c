...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/01/08 16:14:08 by bakpulat          #+#    #+#             */
/*   Updated: 2025/01/10 12:27:40 by bakpulat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	bf[fd] = ft_find_n(bf[fd], fd);
	if (!bf[fd])
	{
		free (bf[fd]);
		return (NULL);
	}
	line = ft_one_line(bf[fd]);
	bf[fd] = ft_after_line(bf[fd]);
	return (line);
}
